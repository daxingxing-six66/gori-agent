...
根启动器、直接启动后端与开发模式均使用此默认目录，不随源码位置改变。先停止程序再备份整个 `.gori-agent` 目录。可用 `SSH_AGENT_DATA_DIR` 指定其他数据目录，升级验证仍见验收记录。显式指定的数据库、本地工作目录和会话目录仍优先使用。提交问题前删除日志中的服务器地址、命令输出和个人信息。
