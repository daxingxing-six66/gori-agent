...
- 数据默认写入 `~/.gori-agent`，可由 `SSH_AGENT_DATA_DIR` 覆盖。数据库文件仍名为 `ssh-agent.sqlite`，密钥为 `credential-key`。已有数据库却缺失密钥、或密钥格式无效时拒绝启动。
- 后端 cwd 设为数据目录，本地工作目录设为其 `workspace/`；日志依旧写入后端包的 `logs/`。启动器不自动迁移旧用户数据。
