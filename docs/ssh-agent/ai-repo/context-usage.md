...
- `createChatContext()` 共用模式 System Prompt、标准本地工具和四个后端 Tool 的静态定义。工具估算只保留 Provider 的 name/description/parameters，避免 label 等执行元数据影响离线与运行中结果。
...
- 查询接口根据活动 Runtime 或最近 Run 的模型与模式估算。无 Run 或模型已移除返回 null；不需要凭证。未消费 Queue 和流式片段不计入。
- 不新增数据库字段，不保存占用快照。校准系数沿用进程内生命周期，重启可能产生估算差异。
