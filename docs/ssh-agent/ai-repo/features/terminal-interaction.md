...
- Chat Run 创建时显式冻结 `serverInteractionMode`。普通模式只暴露 `remote_server_call`，Terminal Mode 移除该 Tool 并暴露 `terminal_interaction`；一个 TerminalSession 同时只绑定一个活动 Run。同一冻结值还选择独立的 Agent System Prompt：稳定共享前缀用 `terminal-model-tag-policy` 说明 `<terminal-model-on>`/`<terminal-model-off>` 是高于历史消息和历史 Tool Result 的权威运行时边界，末尾只追加当前标签。开启标签说明长期有状态 PTY、Observation 和 `terminal_interaction`，关闭标签说明退出 Terminal Mode、`terminal_interaction` 失效并加入 `remote_server_call`。Prompt 不含动态 Run ID且不重写历史消息，以保持缓存前缀稳定。
...
- Terminal Mode 工具是即时 UI Action：条目主标题固定为 `Terminal Mode`，右侧以绿、黄、灰三种呼吸灯分别表示已激活、过渡中和未启动/不可用，具体状态文字只作为无障碍与悬停提示；选择后删除输入框中的 `/...` 或 `@...` 触发文本，直接调用现有 open/close，不发送 Chat 消息，也不创建 Agent Tool Call。连接、断开或活动 Chat Run 时菜单项保留但禁用并说明原因；新 Session 创建前显示“创建会话后可用”。切换继续使用纯前端生命周期投影：开启时展示连接状态，关闭时依次展示断开中、已断开确认和退出阶段；已断开确认短暂停留后，右栏与 Chat 宽度同步过渡。该投影不改变后端状态机、REST/SSE envelope 或领域数据结构，失败时仍回到服务端权威状态并展示原错误。
...
| command/terminal Tool 互斥装配与 Run 绑定协调 | `packages/ssh-agent/src/application/services/chat-agent-runtime-factory.ts`、`packages/ssh-agent/src/application/services/chat-service.ts` |
