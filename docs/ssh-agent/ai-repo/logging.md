...
- 目录固定为包根目录 `logs/`，由模块位置解析，不依赖启动工作目录。源码运行和 dist 运行使用同一目录。目录权限 0700，新日志文件 0600。
...
- 日志可能包含底层异常中的敏感诊断信息，仅用于本机排查。禁止主动记录凭证、请求正文或摘要正文；分享日志前应脱敏。`packages/ssh-agent/.gitignore` 排除整个 logs 目录。
