...
describe("agentLoop with AgentMessage", () => {
	...
	it("preserves structured AgentToolError details in lifecycle events and tool results", async () => {
		const parameters = Type.Object({});
		const tool: AgentTool<typeof parameters, { code: string }> = {
			name: "fail",
			label: "Fail",
			description: "Fail with structured details",
			parameters,
			async execute() {
				throw new AgentToolError({
					message: "Blocked by policy",
					content: [{ type: "text", text: "Safe failure" }],
					details: { code: "policy_blocked" },
					terminate: true,
				});
			},
		};
			...
			queueMicrotask(() => {
				stream.push({
					...
					message: createAssistantMessage([
						{ type: "toolCall", id: "tool-structured", name: "fail", arguments: {} },
					]),
				});
			});
			return stream;
		...
		const events: AgentEvent[] = [];
		const stream = agentLoop([createUserMessage("fail")], context, config, undefined, streamFn);
	...
	it("should suppress automatic continuation after a mixed tool batch", async () => {
		const toolSchema = Type.Object({ value: Type.String() });
		const executed: string[] = [];
		const tool: AgentTool<typeof toolSchema, { value: string }> = {
			name: "echo",
		...
		const config: AgentLoopConfig = {
			...
			toolExecution: "parallel",
			beforeToolCall: async ({ args }) =>
				(args as { value: string }).value === "blocked"
					? { block: true, reason: "Blocked by policy", terminate: true }
					: undefined,
			shouldContinueAutomaticallyAfterTurn: async () => false,
			getSteeringMessages: async () => {
				steeringPolls++;
				return steeringPolls === 1 ? [] : [createUserMessage("steer should not run")];
			},
		};
		let llmCalls = 0;
			...
			() => {
				...
				queueMicrotask(() => {
					mockStream.push({
						...
						message: createAssistantMessage(
							[
								{ type: "toolCall", id: "tool-1", name: "echo", arguments: { value: "blocked" } },
								{ type: "toolCall", id: "tool-2", name: "echo", arguments: { value: "allowed" } },
							],
							"toolUse",
						),
					});
		...
		expect(messages).not.toContainEqual(expect.objectContaining({ content: "steer should not run" }));
	});

	it("should process follow-up after suppressing automatic continuation", async () => {
		let continueAutomatically = false;
		let followUpPending = true;
		const config: AgentLoopConfig = {
			model: createModel(),
			convertToLlm: identityConverter,
			shouldContinueAutomaticallyAfterTurn: async () => {
				const decision = continueAutomatically;
			...
			(_model, context) => {
				...
				queueMicrotask(() => {
					mockStream.push({
						...
						reason: "stop",
						message: createAssistantMessage([
							{ type: "text", text: lastMessage?.role === "user" ? String(lastMessage.content) : "unexpected" },
						]),
					});
				});
				return mockStream;
