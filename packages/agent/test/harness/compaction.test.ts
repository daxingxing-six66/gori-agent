...
describe("harness compaction", () => {
	...
	it("allows an explicit second compaction of the latest retained tail", () => {
		const retainedUser = createUserMessage("retained user ".repeat(200));
		const retainedAssistant = createAssistantMessage("retained assistant ".repeat(200));
		const compaction = createCompactionEntry("first summary", null, [retainedUser, retainedAssistant]);
		...
		const preparation = getOrThrow(
			prepareCompaction(
				...
				{ allowLatestCompaction: true },
			),
		);

		expect(preparation).toMatchObject({ previousSummary: "first summary" });
		expect([
			...(preparation?.messagesToSummarize ?? []),
			...(preparation?.turnPrefixMessages ?? []),
			...(preparation?.retainedTail ?? []),
		]).toEqual([retainedUser, retainedAssistant]);
	});
