...
- Added `AgentToolError` so tools can preserve safe structured failure details in lifecycle events and tool-result messages.
- Added `AgentOptions.shouldContinueAutomaticallyAfterTurn` to stop tool-result and steering continuation while preserving queued follow-up turns.
- Added provider-request preflight context replacement and one-shot recovery hooks, including explicit re-compaction of a latest summary.
