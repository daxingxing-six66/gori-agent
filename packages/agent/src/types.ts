...
/** Context exposed immediately before an LLM provider request. */
...
/** Context exposed for a terminal provider error before that error is committed to the transcript. */
export interface RecoverProviderErrorContext extends BeforeProviderRequestContext {
	message: AssistantMessage;
...
export interface AgentLoopConfig extends SimpleStreamOptions {
	model: Model<any>;
	/** Replace runtime context/model state immediately before every provider request. */
	beforeProviderRequest?: (
		context: BeforeProviderRequestContext,
		signal?: AbortSignal,
	) => AgentLoopTurnUpdate | undefined | Promise<AgentLoopTurnUpdate | undefined>;
	/** Return replacement runtime state to retry one uncommitted terminal provider error. */
	...
	/**
	 * Called after `turn_end` when the loop would otherwise continue from tool calls or steering messages.
	 *
	 * Return false to stop automatic continuation for the completed turn without ending the run. Steering
	 * messages are not polled, but follow-up messages can still start a new turn.
	 *
	 * Contract: must not throw or reject. Throwing interrupts the low-level agent loop without producing a normal event sequence.
	 */
	shouldContinueAutomaticallyAfterTurn?: (context: ShouldStopAfterTurnContext) => boolean | Promise<boolean>;
...
/** Structured failure thrown by a tool when callers need stable details in Agent lifecycle events. */
...
/** Stable failure raised by Agent lifecycle extensions. */
export class AgentRunError extends Error {
	readonly code: AssistantErrorCode | undefined;

	constructor(code: AssistantErrorCode | undefined, message: string, options?: ErrorOptions) {
