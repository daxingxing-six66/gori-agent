...
Use `shouldContinueAutomaticallyAfterTurn` when the completed turn must not continue from tool results or steering, but queued follow-up messages should still be allowed to start a new turn. Returning `false` suppresses the automatic tool loop and skips the steering poll; the normal follow-up poll still runs. The callback runs after `shouldStopAfterTurn`, so a full stop takes precedence.
...
const agent = new Agent({
  ...
  // Stop the current automatic tool chain while still allowing follow-up turns.
  shouldContinueAutomaticallyAfterTurn: async ({ toolResults }, signal) => {
    return !shouldPauseAutomaticContinuation(toolResults, signal);
  },
