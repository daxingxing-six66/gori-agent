...
import { isContextOverflow, isRecoverableLength, normalizeContextOverflowError } from "../src/utils/overflow.ts";
...
describe("isContextOverflow", () => {
	it("normalizes provider-specific overflow text into the standard error code", () => {
		const overflow = createErrorMessage("maximum context length is 32768 tokens");
		const rateLimit = createErrorMessage("Rate limit exceeded, please retry after 30 seconds.");

		expect(normalizeContextOverflowError(overflow, 32768)).toEqual(
			expect.objectContaining({ errorCode: "context_overflow" }),
		);
		expect(normalizeContextOverflowError(rateLimit, 32768)).toBe(rateLimit);
	});
