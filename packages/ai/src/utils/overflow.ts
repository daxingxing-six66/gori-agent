...
/** Add the stable context-overflow code while preserving the provider's original diagnostics. */
export function normalizeContextOverflowError(message: AssistantMessage, contextWindow?: number): AssistantMessage {
	if (message.errorCode === "context_overflow" || !isContextOverflow(message, contextWindow)) return message;
	return { ...message, errorCode: "context_overflow" };
}
